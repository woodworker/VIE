...
test("vie.js Entities API", function () {
  ...
  equal(z.entities.at(0).get('http://purl.org/dc/elements/1.1/title').toString(), 'Bar');

  z.entities.at(0).set({'http://purl.org/dc/elements/1.1/title': 'Baz'});
  equal(z.entities.at(0).get('dc:title').toString(), 'Baz');
  ...
  equal(z.entities.at(0).get('iks:foo').toString(), '');
...
test("vie.js Entities API - addOrUpdate", function () {
  ...
  equal(z.entities.get("http://example.net/foo").get('dc:title').toString(), 'Bar');
  equal(z.entities.get("http://example.net/foo").get('dc:propertyB').toString(), 'Some String');
});
...
test("vie.js Entity API - setOrAdd", function () {
  ...
  clapton.setOrAdd({'plays': 'guitar'});
  equal(typeof clapton.get('plays'), "object");
  ok(clapton.get('plays').isLiteralCollection);

  clapton.setOrAdd({'plays': 'vocals'});
  ok(clapton.get('plays').isLiteralCollection);
  equal(clapton.get('plays').size(), 2);

  clapton.setOrAdd({'plays': 'vocals'});
  equal(clapton.get('plays').size(), 2, "Same value twice is the same value and needs to be only once.");
});
...
test("vie.js Language Setter/Getter", function () {
  ...
  v.setLang("en-US");
  equal(v.getLang(), "en-US");

  v.setLang("adwadawdwad");
  equal(v.getLang(), "en-US");
  
  var v2 = new VIE({lang: "de-AT"});

  equal(v2.getLang(), "de-AT");
  v2.setLang("en");
  equal(v2.getLang(), "en");
