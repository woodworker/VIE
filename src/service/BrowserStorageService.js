...
VIE.prototype.BrowserStorageService.prototype = {
	...
	removeFile: function(fileName){
		this.connector.removeFile(fileName);
		return this;
	},
    ...
    load: function(loadable){
		...
		if (entities || loadAll) {
            this.connector.load(success, error);
        }
		else {
            loadable.reject([]);
        }
...
VIE.prototype.BrowserStorageConnector.prototype = {
	...
	load: function (success, error) {
		...
		window.webkitStorageInfo.requestQuota(PERSISTENT, storageSize, function(grantedBytes) {
			window.requestFileSystem(PERSISTENT, grantedBytes, function onInitFs(fs) { fs.root.getFile(fileName, {}, function(fileEntry) {

				fileEntry.file(function(file) {
					var reader = new FileReader();
					reader.onloadend = function(e) {
						var entities = JSON.parse(this.result);
						success(entities);
					};
	...
	removeFile: function(){
		var fileName = this.options.fileName;
		window.requestFileSystem(PERSISTENT, this.options.storageSize, function onInitFs(fs) { fs.root.getFile(fileName, {create: true}, function(fileEntry) {
